<?php

declare(strict_types=1);

namespace App\Infrastructure\Persistence;

/**
 *
 * DBPDO LIBRARY FOR PHP - VERSION 1.0 - http://ignacioxd.com/
 *
 * Copyright (C) Ignacio X. Domínguez
 *
 * For more information, visit http://ignacioxd.com/
 *
 * This notice may not be removed or altered from any source distribution.
 * This software is provided 'as-is', without any express or implied warranty.
 * In no event will the author be held liable for any damages arising from the
 * use of this software. Permission is granted to anyone to use this software
 * for any purpose, including commercial applications subject to the following
 * restrictions:
 *
 * 1. The origin of this software must not be misrepresented; you must not claim
 * that you wrote the original software. If you use this software in a product,
 * an acknowledgment in the product would be appreciated but is not required.
 *
 * 2. Altered source versions must be plainly marked as such, and must not be
 * misrepresented as being the original software.
 *
 * 3. This notice may not be removed or altered from any source distribution.
 *
 */

/**
 * Description of DBPool
 *
 * @author ignacioxd
 */
class DBPool
{
    private $dbSettings;
    private $pool = array();

    public function __construct(array $dbSettings)
    {
        $this->dbSettings = $dbSettings;
    }

    /**
     *
     * @return DBPDO
     * @throws Exception
     */
    public function request(): DBPDO
    {
        if (count($this->pool) > 0) {
            $item = array_rand($this->pool);
            $DBConn = $this->pool[$item];
            unset($this->pool[$item]);
            return $DBConn;
        }
        $DBConn = new DBPDO($this->dbSettings);
        if (!$DBConn) {
            throw new \Exception("Unable to obtain a connection to the database", 0);
        }
        return $DBConn;
    }

    public function release(DBPDO $DBConn): void
    {
        $this->pool[] = $DBConn;
    }

    public function purge(): void
    {
        foreach ($this->pool as $index => $DBConn) {
            $DBConn = $this->pool[$index];
            unset($this->pool[$index]);
            $DBConn->disconnect();
            $DBConn = null;
        }
    }
}
